...
{{-- @if(Auth::check())
    ...
    <p>Family Page ID: {{ Auth::user()->family_page_id ?? 'null' }}</p>
@else
    <p>Logged In: No</p>
@endif --}}
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-4">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                ...
                @if(session('noAccessToFamilyPage'))
                <div class="mt-4 p-4 bg-yellow-200 text-yellow-800 border-l-4 border-yellow-600">
                <p>{{ __('注意: アクセスできる相続手続きのためのファミリーページが見当たりません。') }}</p>
            </div>
            @endif
        </div>
    </div>
    ...
    {{-- ここからボタン --}}
            <div class="mt-8 px-8">
                ...
                <div class="scale-100 p-6 bg-amber-100 dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500">
                    
                    <div>
                        <div class="h-16 w-16 bg-white dark:bg-red-800/20 flex items-center justify-center rounded-full">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" class="w-7 h-7 stroke-red-500">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6.115 5.19l.319 1.913A6 6 0 008.11 10.36L9.75 12l-.387.775c-.217.433-.132.956.21 1.298l1.348 1.348c.21.21.329.497.329.795v1.089c0 .426.24.815.622 1.006l.153.076c.433.217.956.132 1.298-.21l.723-.723a8.7 8.7 0 002.288-4.042 1.087 1.087 0 00-.358-1.099l-1.33-1.108c-.251-.21-.582-.299-.905-.245l-1.17.195a1.125 1.125 0 01-.98-.314l-.295-.295a1.125 1.125 0 010-1.591l.13-.132a1.125 1.125 0 011.3-.21l.603.302a.809.809 0 001.086-1.086L14.25 7.5l1.256-.837a4.5 4.5 0 001.528-1.732l.146-.292M6.115 5.19A9 9 0 1017.18 4.64M6.115 5.19A8.965 8.965 0 0112 3c1.929 0 3.716.607 5.18 1.64" />
                            </svg>
                        </div>
                            @if(Auth::check() && Auth::user()->familyPages->isNotEmpty())
                                <a href="{{ route('family_pages.show', ['id' => Auth::user()->familyPages->first()->id]) }}" class="btn btn-primary mt-6 text-xl font-semibold text-gray-900 dark:text-white">ファミリーページ</a>
                                <p class="mt-4 text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                                    ご家族専用の相続手続きページ。（有料）
                                </p>
                            @else
                                <!-- ユーザーが未認証、もしくはfamily_page_idが設定されていない場合のメッセージやリンク -->
                                <p class="mt-4 text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                                    お客様のファミリーページを確認するには、<a href="{{ route('login') }}" class="text-blue-500 underline">ログイン</a>が必要です。
                                </p>
                            @endif
                    </div>
                </div>
                ...
                <div class="scale-100 p-6 bg-amber-100 dark:bg-gray-800/50 dark:bg-gradient-to-bl from-gray-700/50 via-transparent dark:ring-1 dark:ring-inset dark:ring-white/5 rounded-lg shadow-2xl shadow-gray-500/20 dark:shadow-none flex motion-safe:hover:scale-[1.01] transition-all duration-250 focus:outline focus:outline-2 focus:outline-red-500">
                    <div>
                        <div class="h-16 w-16 bg-white dark:bg-red-800/20 flex items-center justify-center rounded-full">
                            ...
                            <a href="{{ route('deceased_persons.create') }}" class="btn btn-primary mt-6 text-xl font-semibold text-gray-900 dark:text-white">手続きページを作成</a>
                            <p class="mt-4 text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
                            どのような手続きが必要か細かく診断することができます。（有料）
                        </p>
                    </div>
                </div>
